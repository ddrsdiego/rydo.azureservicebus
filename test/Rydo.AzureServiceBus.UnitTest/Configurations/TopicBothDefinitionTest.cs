namespace Rydo.AzureServiceBus.UnitTest.Configurations
{
    ...
    public class TopicBothDefinitionTest
    {
        [Test]
        public void Should_Create_Valid_TopicBothDefinition_With_Default_Values()
        {
            const string topicName = "topic-name";
            var subscriptionName = typeof(AccountCreatedMessage).Assembly.GetName().Name.ToLowerInvariant();

            var topicProducerDefinition = new TopicBothDefinition(topicName, subscriptionName);

            topicProducerDefinition.TopicName.Should().Be(topicName);
            topicProducerDefinition.SubscriptionName.Should().Be(subscriptionName);
            topicProducerDefinition.Direction.Should().Be(TopicDirections.Both);
        ...
        [Test]
        public void Should_Create_Valid_TopicBothDefinition()
        {
            const string topicName = "topic-name";
            var subscriptionName = typeof(AccountCreatedMessage).Assembly.GetName().Name.ToLowerInvariant();

            var topicProducerDefinition = new TopicBothDefinition(topicName, subscriptionName, 120, 5);
